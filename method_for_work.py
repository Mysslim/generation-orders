...
def put_volume_in_quotes(volume):
    return "'{}'".format(str(volume))

def make_str_enum_for_dump(volumes):
    output_str = "ENUM("
    for volume in volumes:
        output_str += put_volume_in_quotes(volume) + ","
    
    output_str = output_str[:-1] + ")"
    
    return output_str

def sql_query_make_table(config):
    list_quotation = []
    

def write_to_dump(history_records, config):
    dump = []
    name_of_columns = get_attribute_of_tables(config)
    name_DB = config["volume_for_dump"]["DATABASE"]
    name_table = config["volume_for_dump"]["TABLE"]
